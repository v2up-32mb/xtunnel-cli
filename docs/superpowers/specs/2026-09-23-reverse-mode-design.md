...
| `MsgHotPairNotify` (0x22) | 客户端→服务端 | 批量预热通道对通知：payload = 记录 × N（`[1B keyLen][key][4B ChA(client→server)][4B ChB(server→client)]`），一条消息携带多条 HotPair，无总数前缀按长度自然终止 |
...
### HotPair 预热路径 v2：双端热表 + 前缀 connID（-hotpair 启用时）

**v3 修订（双端热表）**：健康维护（预热/刷新/失效/重建）统一由**客户端**负责，正反两模式共用同一套机制——预热完成后经 `MsgHotPairNotify (0x22)` 把通道对**批量通知**服务端，双方各维护一张 HotPair 表；代理请求到达时拨号方查表取 Pair，**connID = 预热键 + '.' + 每连接唯一后缀**，接收方按前缀查表直接获得完整收发通道，拨号期零选路消息。Pair **共享复用**（同一对通道服务多条并发连接，后缀保证 connID 互不冲突），表项长存直到通道失效/TTL。

```
预热(客户端 PairWarmer，三步完成全部选路，正反模式同一套):
  ① 客户端广播 MsgPrebindRequest("prebind-x", 复用 0x10)
  ② 服务端通道 X 首达占用 → 广播 MsgSelectUplink("prebind-x",[X])（不拨号，即注注销）
  ③ 客户端竞争收包，最快通道 Y = RX → Pair{TX:X(client→server), RX:Y(server→client)} Ready
  ④ MsgHotPairNotify("prebind-x", ChA=TX, ChB=RX) → 服务端按键 upsert 热表
     （新 Pair 500ms 防抖合批；周期刷新对存量 Ready Pair 心跳重发，刷新服务端表项时间戳）
     至此双方热表均持有完整通道对，上下行竞争全部完成。

正向请求时(客户端=拨号方):
  查本地热表取 Ready Pair → connID = "prebind-x" + "." + uuid
  → MsgTCPConnect(connID,[策略,target]) 单播 TX（拨号期零选路消息，预置 st.uplink）
  → 服务端按前缀查表：recvCh=到达通道(=ChA)，downlink=ChB，跳过 MsgSelectUplink 广播
  → 连接目标 → MsgConnStatus OK（单播 ChB）。Pair 共享复用，服务无限连接。

反向请求时(服务端=拨号方):
  查热表取最新 Pair（双通道活性校验）→ connID = "prebind-x" + "." + uuid
  → MsgTCPConnect(connID,[策略,target]) 单播 ChB（拨号期零选路消息）
  → 客户端按前缀查表：recvCh=到达通道(=ChB)，sendCh=ChA，不广播 MsgSelectUplink
  → 连接目标 → MsgConnStatus OK。Pair 共享复用。

回退与兜底:
  无就绪表项 / 单播发送失败 → 回退 §4b 广播+竞争流程（经典路径自愈）
  接收方表项缺失/不匹配（通知在途、表项被清扫）→ 落回经典竞争路径；
    预热热路径已预置通道的（反向 pairPreset）允许一次性兜底修复：
    采用客户端 MsgSelectUplink 实际收发通道并幂等补发一次 MsgSelectDownlink
  通道断开 → 两端联动清扫引用该通道的表项；客户端全掉线 → 清空其全部表项
  表项 TTL（10min）最后防线：健康 Pair 由心跳重发续期
```

**为何 connID 携带键前缀（一石三鸟）**：
1. **多条 Pair 消歧**：查表按键精确前缀匹配而非到达通道，多条 Pair 共享同一通道也能唯一定位；
2. **广播回退安全**：回退拨号用裸 uuid connID（无前缀），接收方绝不查表——不会把广播副本误当热路径消费（消除了缓存方案的错配竞态）；
3. **并发唯一性**：后缀每连接唯一，Pair 共享复用不会撞 connID（正向 connID 复用曾实测 8 路并发仅 2 路成功的教训，以"键.后缀"形式修正后回归）。

**为何预热统一由客户端做**：与 `-l`/`-hotpair` 一致的控制权归属（客户端启动参数决定，服务端零配置）；预热是通道健康探测，通道池生命周期归客户端管理（重连/网络切换/息屏）；预热结果经通知帧交付，服务端纯被动存表（`HotPairTable`：HandleNotify/Lookup/Newest/Invalidate）。旧服务端 switch 无此 case 自动忽略（无热表，拨号回退经典路径，功能不受影响）；旧客户端不发通知，服务端表恒空。
...
图例：〔B〕=广播；〔ChB〕〔通道N〕=单播；时间自上而下。通道以绝对方向命名：ChA=client→server（客户端发包），ChB=server→client（服务端发包）。预热键 `prebind-x`。
...
─────────────────────────────────────────────────────────────────────────────────
 ① 启动
        ...
        │                                                        │
 ② 监听 │ ── MsgReverseListen(lid,"socks5://…:28180") 〔通道N〕 ─▶ │ 绑定 127.0.0.1:28180（幂等）
        │ ◀───── MsgReverseListenResult(OK) 〔同通道〕 ─────────── │ 日志：反向监听已开启
        │                                                        │
 ③ Pair │ ── ①MsgPrebindRequest("prebind-x",[策略,prebind]) 〔B〕─▶│ 首达占用、回 ② 后立即注销（不拨号）
   预热 │ ◀── ②MsgSelectUplink("prebind-x",[X]) 〔B〕 ──────────── │ X = ChA（client→server 最快）
        │ 竞争：最快到达通道 Y = ChB                              │
        │ Pair{TX=X, RX=Y} Ready（共享复用，长存）                 │
        │ ── ④MsgHotPairNotify("prebind-x", ChA=X, ChB=Y) ──────▶ │ 热表 upsert（可多条合批）
        │ （周期刷新：心跳重发存量 Pair 通知，续期服务端表项）        │ 日志：收到预热通道对通知
        │ （至此双端热表均持有完整通道对，竞争全部完成）             │
        │                                                        │
 ④ 代理 │                                                 curl ──▶│ SOCKS5 :28180 accept（握手在服务端本地）
   请求 │                                                        │ 查热表取 Pair（活性校验）
        │ ◀── MsgTCPConnect("prebind-x.<uuid>",[策略,target]) ─── │ connID=键.后缀；单播 ChB（零选路消息）
        │ 按前缀查表 → recvCh=ChB、sendCh=ChA 全知                 │ 单播直达（免竞争，首帧 ≈1 RTT）
        │ net.DialTimeout(target) → 成功                          │ 启动上行泵（管道→ChB）
        │ ── MsgConnStatus(connID,OK) ──────────────────────────▶ │ settle → DialStream 返回管道 conn
        │                                                        │ xshared 双向 io.Copy（用户 ↔ conn）
        │ ◀──────── 上行 MsgTCPData 〔ChB〕 ───────────────────── │ 用户请求 → 管道 → 上行泵
        │ 校验 ch==ChB → 写本地 conn ──▶ 目标                      │
        │ ◀──────── 下行 MsgTCPData 〔ChA〕 ─────────────────────▶│ 目标响应 → 客户端读泵(sendCh=ChA)
        │ 目标 ──▶ 读泵                                          │ 校验 ch==ChA → 写管道 → 用户
        │                                                        │
 ⑤ 连接 │ curl 结束 → SOCKS5 拆隧道 → rc.app 关闭                  │
   关闭 │ ◀── MsgTCPClose(connID) 〔ChB〕 ─────────────────────── │ 上行泵管道 EOF → 通知并清理
        │ 关本地 conn，反向表删除                                  │ 反向表删除
        │ （若目标先关：客户端 ── MsgTCPClose 〔ChA〕 ──▶ 服务端清理）│
        │                                                        │
...
- 无 `-hotpair` 时：跳过 ③ 整个阶段（双方热表恒空）；④ 服务端无表项可取，回退 §4c 广播竞争流程（裸 uuid connID）。
- 接收方表项缺失/不匹配（通知在途、表项被清扫）：④ 落回经典竞争路径；反向预热热路径已预置通道的（pairPreset）允许一次性兜底修复——采用客户端 `MsgSelectUplink` 实际收发通道并幂等补发一次 `MsgSelectDownlink`，功能不中断。
- Pair 共享复用：多条并发连接共用同一对通道，后缀保证 connID 互不冲突；表项 TTL（10min）由心跳重发续期。
- 通道中途死亡：两端 `cleanupChannel` 清扫引用该通道的热表表项与反向连接（对齐 §4c 兜底）。
...
> 注（v3 修订）：HotPair 健康维护统一由客户端负责——`xtunnel` 库 PairWarmer 预热完成后经 `MsgHotPairNotify` 通知服务端建热表；CLI 服务端改为被动存表（见 6.2）。

| 文件 | 改动 |
|---|---|
| `protocol/protocol.go` | + `MsgReverseListen`/`MsgReverseListenResult`/`MsgHotPairNotify` 常量（仅常量，格式不变）；+ `HotPairInfo` 及批量编解码、`HotPairConnID`/`SplitHotPairConnID` connID 前缀约定 |
...
| `hotpair_table.go`（新） | 服务端预热通道对表（被动）：接收客户端 `MsgHotPairNotify` 批量通知（按键 upsert + TTL 清扫），供正向 `handleTCPConnect` 按键提升（`Lookup`）与反向 `DialStream` 择新拨号（`Newest`）；`InvalidateChannel`/`InvalidateClient` 联动清扫 |
...
- `xtunnel` 库 ~400 行 + 测试；CLI 服务端 ~500 行 + 测试（含被动 `HotPairTable` ~150 行）；CLI 客户端 ~50 行。
