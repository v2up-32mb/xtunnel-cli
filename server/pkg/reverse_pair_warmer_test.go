...
// TestPrebindWarmPromotion 正向预热 Pair 消费：拨号期零选路消息。
// 预热三步（PrebindRequest→SelectUplink→SelectDownlink）后，客户端以同一 connID
// 发 MsgTCPConnect，服务端直接提升 warm 状态（不广播 MsgSelectUplink、不等竞争）。
func TestPrebindWarmPromotion(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Token = "token"
	p := newServerPool(cfg.Token, cfg)
	server := httptest.NewServer(http.HandlerFunc(p.handleWebSocket))
	...
	// ① 预热请求（客户端广播）
	connID := "prebind-warm-fwd"
	meta := []byte{0}
	meta = append(meta, protocol.PrebindTarget...)
	p.handleMessage("client-a", 1, len(meta)+8, protocol.MsgPrebindRequest, connID, meta, nil)

	// 服务端广播 MsgSelectUplink([1])（通道 1 首达占用）
	mtype, gotID, gotMeta, _ := readFrameType(t, conn1, 3*time.Second)
	...
	mtype2, gotID2, gotMeta2, _ := readFrameType(t, conn2, 3*time.Second)
	if mtype2 != protocol.MsgSelectUplink || gotID2 != connID || binary.BigEndian.Uint32(gotMeta2) != 1 {
		t.Fatalf("expected broadcast copy on ch2, got type=%d", mtype2)
	}

	// ② 客户端完成下行竞争（选定通道 2），经上行通道 U=1 回 MsgSelectDownlink([2])
	if err := conn1.WriteMessage(websocket.BinaryMessage, protocol.EncodeMessage(protocol.MsgSelectDownlink, connID, be32v(2), nil)); err != nil {
		t.Fatalf("write warmup MsgSelectDownlink: %v", err)
	}
	time.Sleep(100 * time.Millisecond) // 等服务端处理，补全 warm 状态
	p.mu.RLock()
	st := p.conns[connID]
	downlink := 0
	if st != nil {
	...
	p.mu.RUnlock()
	if downlink != 2 {
		t.Fatalf("warm downlink should be 2, got %d", downlink)
	}

	// ③ 拨号期：以同一 connID 发 MsgTCPConnect（复用 prebind connID）
	dialMeta := []byte{0}
	dialMeta = append(dialMeta, []byte("127.0.0.1:1")...) // 拨号必失败，快速回 ERR
	p.handleMessage("client-a", 1, len(dialMeta)+8, protocol.MsgTCPConnect, connID, dialMeta, nil)

	// 服务端不应广播 MsgSelectUplink（提升路径零选路消息）；
	// ConnStatus 经预热下行通道 D=2 单播直达，非下行通道应零帧
	mtype3, id3, gotMeta3, _ := readFrameType(t, conn2, 3*time.Second)
	if mtype3 != protocol.MsgConnStatus || id3 != connID || gotMeta3[0] != byte(protocol.StatusERR) {
		t.Fatalf("expected MsgConnStatus(ERR) on ch2 (downlink), got type=%d id=%s meta=%v", mtype3, id3, gotMeta3)
	}
	_ = conn1.SetReadDeadline(time.Now().Add(400 * time.Millisecond))
	if _, _, err := conn1.ReadMessage(); err == nil {
		t.Fatal("non-downlink channel should receive nothing (no selection broadcast)")
	}

	// 拨号失败后连接状态应被清理（提升路径复用标准 connectTarget 清理流程）
	p.mu.RLock()
	_, stillExists := p.conns[connID]
	p.mu.RUnlock()
	if stillExists {
		t.Fatal("failed connection should be cleaned up")
	}
}
