...
type prebindRacer struct {
	chs    []int // 收到该预绑定 MsgSelectUplink 的通道
...
func (p *clientPool) noteUplink(connID string, chID int) {
	...
	if st.uplink == 0 {
		...
		// clientLogf("[客户端] 警告: 连接 %s 的上行通道从 %d 变更为 %d", common.ShortID(connID), st.uplink, chID)
	}
	p.mu.Unlock()
}
...
func (p *clientPool) handleChannel(chID int, conn *websocket.Conn) {
	...
	for {
		...
		case common.MsgSelectUplink:
			...
			// 收帧窗口 400ms 后由 AfterFunc 汇总打印一次（每个 connID 仅一次）
